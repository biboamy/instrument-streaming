...
def main(argv):
    ...
    pred_roll = np.zeros((s[0],num_cat,88,s[2]))
    #start predict
    ...
    pred_inst = all_pred = np.transpose(pred_inst, (1, 0, 2)).reshape((num_cat,-1))
    pred_pitch = np.transpose(pred_pitch, (1, 0, 2)).reshape((88,-1))
    pred_roll = np.transpose(pred_roll, (1, 2, 0, 3)).reshape((num_cat,88,-1))
